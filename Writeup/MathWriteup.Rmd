...
The learning dynamics can be evaluated analytically and numerically using a mathematical model. Following [citation needed], this model assumes that learning happens at a much faster rate than evolution, so the equilibirium behaviors from learning can be calculated and then used to determine the resulting payoffs to the different learning strategies.

Individual learners just learn whether to respond to an alarm and the absence of an alarm based on whether they detect a predator.
$$ P(resp|alarm,I) = P(alarm|predator)P(predator)P(detect) $$
$$ P(resp|noAlarm,I) = (1-P(alarm|predator))P(predator)P(detect) $$

Social learners learn whether to respond to an alarm and the absence of an alarm based on whether they detect a predator or, if they don't detect a predator, the response of a conspecific (either a social learner or individual learner).
\begin{align*}
